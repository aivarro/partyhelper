...
public class PartyService {
    ...
    public PartyService(ChatClient.Builder chatClientBuilder) {
        this.chatClient = chatClientBuilder.build();
    }

    /**
     * Põhimeetod, mis viib läbi turvakontrolli ja küsib tehisintellektilt peosoovituse.
     */
    public PartyDecision getPartyRecommendation(UserInput input) {

        String combinedInput = input.mood() + " " + input.preferences();

        // 1. STAATILINE KONTROLL
        if (INJECTION_PATTERN.matcher(combinedInput).find()) {
            log.warn("Staatiline filter blokeeris sisendi: {}", combinedInput);
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Ebasobiv sisend!");
        }

        // 2. LLM KOHTUNIK - Turvakontroll
        ...
        // 3. PEONÕUSTAJA - Generatiivne loogika
